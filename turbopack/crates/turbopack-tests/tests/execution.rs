...
use turbo_tasks::{
    debug::ValueDebugFormat, fxindexmap, trace::TraceRawVcs, Completion, OperationVc, RcStr,
    ResolvedVc, TryJoinIterExt, TurboTasks, Value, Vc,
};
...
async fn run(resource: PathBuf, snapshot_mode: IssueSnapshotMode) -> Result<JsResult> {
    ...
    tt.run_once(async move {
        ...
        if matches!(snapshot_mode, IssueSnapshotMode::Snapshots) {
            snapshot_issues(prepared_test, OperationVc::new(run_result)).await?;
        }
...
async fn snapshot_issues(
    ...
    run_operation: OperationVc<RunTestResult>,
) -> Result<Vc<()>> {
